...
It only adds titles to Markdown export when src_emacs-lisp[:exports code]{org-md-title} is non-nil:
...
First, it advises src_emacs-lisp[:exports code]{org-md-template} by prepending the document title.
...
Whenever that function is called, the advise kicks in and increments the second argument with 1. if src_emacs-lisp[:exports code]{org-md-title} is enabled[fn:h2]:
...
After calling src_emacs-lisp[:exports code]{org-md-title-add}, set thte src_emacs-lisp[:exports code]{org-md-title} variable to add document titles when exporting with ox-md:
