...
Even though Org documents can have titles through the src_org[:exports code]{+#title:} export setting, ox-md produces Markdown files with the titles omitted.
For example, the current document has export settings, including a title:

#+include: "ox-md-title.org" src org :lines "1-7"

Exporting to markdown with =org-md-export-as-markdown= produces a document without a title headline:
...
#+begin_src emacs-lisp
  ...
  (let ((contents (buffer-substring 1 146)))
    (kill-buffer)
    contents)
#+end_src

#+RESULTS[bdca1232b792702a2b1996b528ca163226522ccc]:
#+begin_src markdown
[Ox-md-title.el](https://github.com/jeffkreeftmeijer/ox-md-title.el) adds document titles to markdown files generated with ox-md and derivatives.
#+end_src
