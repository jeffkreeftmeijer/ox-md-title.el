...
#+include: "ox-md-title.org" src org :lines "1-9"
...
The second advice intercepts calls to =org-md-headline=.
It increments the headline level by one if =org-md-title= is enabled and if the current document has a title set before calling the original function:
